...
def getAvgHistoryScore(entryId):
    url = 'https://fantasy.premierleague.com/api/entry/' + \
        str(entryId) + '/history/'
    r = requests.get(url).json()
    avg = (r['past'][-1]['rank'] + r['past'][-2]
           ['rank'] + r['past'][-3]['rank']) / 3
...
def index():
    ...
    if not r['standings']['results']:
        ...
        result = render_template(
            'warm_up.html', data=data, getAvgHistoryScore=getAvgHistoryScore)
        return result
    ...
    # Auto subs
    def getGwFixtures():
        url2 = 'https://fantasy.premierleague.com/api/fixtures/?event=' + \
            str(thisGw)
        r2 = requests.get(url2)
        json2 = r2.json()
    ...
    def getMinutesPlayed():
        url1 = 'https://fantasy.premierleague.com/api/event/' + \
            str(thisGw) + '/live/'
    ...
    def getAutoSubs(teamId):
        url4 = 'https://fantasy.premierleague.com/api/entry/' + \
            str(teamId) + '/event/' + str(thisGw) + '/picks/'
        r4 = requests.get(url4)
        json4 = r4.json()
        picks_df = pd.DataFrame(json4['picks'])

        spillerListeOrg = picks_df[[
            'element', 'multiplier', 'is_captain', 'is_vice_captain']]
        ...
        for i in range(len(spillerListe[0:11])):
            ...
            if spilteIkke and erKaptein:
                spillerListe.loc[spillerListe['is_vice_captain']
                                 == True, 'multiplier'] = spillerListe.iat[i, 1]
            ...
            if spillerpos == gk and spilteIkke:
                ...
                if not didNotPlay(keeperbytte):
                    spillerListe.iat[i, 0], spillerListe.iat[11,
                                                             0] = spillerListe.iat[11, 0], spillerListe.iat[i, 0]
            ...
            if spillerpos != gk and spilteIkke:
                ...
                for j in range(len(spillerListe[12:15])):
                    ...
                    innbytterPos = teams.at[spillerListe[12:15].iat[j,
                                                                    0], 'element_type']

                    if countDef >= minDef and countMid >= minMid and countAtt >= minAtt:
                        spillerListe.iat[i, 0], spillerListe[12:15].iat[j,
                                                                        0] = spillerListe[12:15].iat[j, 0], spillerListe.iat[i, 0]
                        spillerListe.iat[i, 1] = 1
        ...
        for i in range(len(fixtures_df)):
            ...
            if now > played60:
                try:
                    ...
                    samlet = stats_a.concat(stats_h)
                    sort = samlet.sort_values(
                        by=['value'], ascending=False)
                    ferdig = sort.reset_index(drop=True)
                    bps = ferdig[0:8].copy()
                    elements = elements.append(
                        bps, ignore_index=True, sort=False)
    ...
    def getAllPlayerList():
        ...
        stats_df = pd.DataFrame(liveElements_df['stats'].values.tolist())
        liveTotPoints_df = pd.DataFrame(
            stats_df[['total_points', 'bonus']])
    ...
    def getGwRoundPoints(teamId):
        ...
        livePlayerPoints_trans = livePlayerPoints - \
            teamPoints_df['event_transfers_cost'][thisGw-1]
        ...
        if thisGw == 1:
            ...
            liveRound = teamPoints_df['total_points'][(
                thisGw - 2)] + livePlayerPoints_trans

            liveTotal = teamPoints_df['total_points'][(
                thisGw - 2)] + livePlayerPoints_trans
    ...
    def getTeamsPoints():
        ...
        ny_tabell = tabell_df.rename(
            columns={0: "Total", 1: "GWLive", 2: "Round"})
        return ny_tabell
    ...
    def getTabell():
        ...
        tabellSort.columns = ['Rank', 'GWround',
                              'Tot', 'GW', 'Navn', 'Entry']

        return tabellSort

    def getCap(playerId):
        url2 = 'https://fantasy.premierleague.com/api/entry/' + \
            str(playerId) + '/event/' + str(thisGw) + '/picks/'
        r2 = requests.get(url2)
        picks = r2.json()['picks']
    ...
    def getLastRoundPoints(teamID):
        ...
        LastRoundPoints = r[thisGw - 2]['total_points'] - \
            r[gws - 2]['total_points']
        return LastRoundPoints

    def genereateLastPlacements():
        lastStandings = []
        for manager in data:
            lastStandings.append(
                (getLastRoundPoints(manager['Entry']), manager['Entry']))
    ...
    result = render_template('main_page.html', data=data, gwHead=gwHead, thisGw=thisGw, getChip=getChip,
                             getCap=getCap, countFinishedPlayers=countFinishedPlayers, upOrDown=upOrDown)
...
def vinnere():
    ...
    def getRoundWinners(roundStart, roundEnd):
        ...
        for i in range(len(teamsList)):
            url = 'https://fantasy.premierleague.com/api/entry/' + \
                str(teamsList.at[i, 'entry']) + '/history/'
            teamPoints = requests.get(url).json()['current']
            if roundEnd == 4:
                if high < teamPoints[roundEnd - 1]['total_points']:
                    result = (
                        teamPoints[roundEnd - 1]['total_points'], teamsList.at[i, 'player_name'])
    ...
    def getWinners():
        ...
        nyRunde = [(1, 4), (5, 8), (9, 12), (13, 16), (17, 20),
...
def lag(lagId):
    ...
    def getAutoSubs(teamId):
        ...
        spillerListe = spillerListe[['element',
                                     'multiplier', 'byttet_ut', 'byttet_inn']]
    ...
    def getPlayerInfo(playerId):
        url2 = 'https://fantasy.premierleague.com/api/event/' + \
            str(thisGw) + '/live/'
        ...
        for i in range(len(df)):
            ...
            if tempIdentifier not in visited and tempIdentifier != 'bonus':
                tempValue = df.loc[df['identifier']
                                   == tempIdentifier, 'value'].sum()
                test.append({
        ...
        try:
            if bonuspoints.at[playerId] > 0:
                test.append(
                    {'identifier': 'bonus', 'points': bonuspoints.at[playerId], 'value': bonuspoints.at[playerId]})
        except:
            pass
        return test
    ...
    result = render_template('lag.html', data=data, poeng=poeng,
                             manager=manager, chip=chip, countPlayed=countPlayed)
